...
#Search Function for uninformed search
...
#queing function based on the search algorithms	
def queuing_fn(search_algo,parent,children):
	global node_list
	if(search_algo == "dfs"):#for dfs, add children to front
		node_list.extendleft(children)
	elif(search_algo == "bfs"):#for bfs, add children to last
...
#Function to get valid successor state for a parent state	
...
#Function to verify  whether child is already visited
...
#Function to verify whether a movement is valid from a state	
...
#Function to return a new state depending on the move
...
#Function to parse the arguments given by the user
def parse(arg):
...
#Find children depending on the heuristic
...
#Find the heuristic value for h1 (out of place tiles) h2(manhattan distance)
...
#Function to print the path once the goal is reached
def printPath():
	goalStateStr=''.join(str(e) for e in GOAL_STATE);
        temp=goalStateStr
	directions=[]
